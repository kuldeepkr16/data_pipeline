...
export const SourceForm: React.FC<SourceFormProps> = ({ initialData, onSubmit, onCancel, isEditing = false }) => {
    ...
    const handleTestConnection = async () => {
        ...
        // Frontend Validation
        const activeDefinition = getConnectorDefinition(formData.source_type || defaultType);
        if (activeDefinition) {
            const missingFields: string[] = [];
            activeDefinition.fields.forEach(field => {
                if (field.required) {
                    const val = formData.source_creds?.[field.key];
            ...
            if (missingFields.length > 0) {
                setTestStatus('idle'); // Reset status so they can try again immediately
    ...
    return (
        <div className="space-y-6">
            <div className="space-y-4">
                    ...
                    <select
                        ...
                        onChange={(e) => {
                            setFormData({
                                ...
                                source_creds: {} // Reset creds on type change
                            });
                            setTestStatus('idle');
                        }}
                        disabled={isEditing}
                ...
                <div>
                    ...
                    <Input
                        ...
                        placeholder="e.g., prod_data_source"
                    />
                </div>
                        ...
                        <div className="grid grid-cols-1 gap-4">
                            {activeDefinition.fields.map(field => (
                                <div key={field.key}>
                                    ...
                                    {field.type === 'textarea' ? (
                                        <textarea
                                            className="w-full bg-[#0f111a] border border-white/10 rounded-lg px-4 py-2 text-white placeholder-gray-600 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-colors h-24 font-mono text-xs"
                                            value={formData.source_creds?.[field.key] || ''}
            ...
            <div className="flex justify-end gap-3 pt-4 border-t border-white/5">
                ...
                >
                    {loading ? 'Saving...' : isEditing ? 'Save Changes' : 'Create Source'}
                </Button>
            </div>
        </div>
    );
