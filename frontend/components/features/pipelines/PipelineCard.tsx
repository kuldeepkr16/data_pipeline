...
import { Config, PipelineRun } from '../../../types';
...
export const PipelineCard: React.FC<PipelineCardProps> = ({
    config, latestRun, loadedStats, triggeringTable, selectedTimeRange,
    onTrigger, onToggleStatus, onEdit, onDelete, disableNavigation = false
}) => {
    ...
    const handleCardClick = (e: React.MouseEvent) => {
        ...
        router.push(`/pipelines/${config.source_tablename}`);
    };

    const Icons = {
        Postgres: (
            <svg className="w-5 h-5" viewBox="-4 0 264 264" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" fill="#000000"><g id="SVGRepo_bgCarrier" strokeWidth="0"></g><g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g><g id="SVGRepo_iconCarrier">
    ...
    return (
        <div
            className={`bg-gray-900/60 border border-white/10 rounded-xl overflow-hidden backdrop-blur-md hover:border-indigo-500/30 transition-all group ${!disableNavigation ? 'cursor-pointer' : ''}`}
        ...
        >
            ...
            <div className="p-4 flex flex-col md:flex-row items-center justify-between gap-4 border-b border-white/5 bg-black/20">
                <div className="flex items-center gap-6 w-full md:w-auto">
                    {/* ID Badge - Using hash of name or simplified index not available here, optional */}
                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div className="flex items-center gap-1.5 px-2 py-1 rounded bg-white/5 border border-white/5 group-hover:bg-white/10 transition-colors">
                                {config.source_type === 'postgres' ? Icons.Postgres : Icons.Source}
                            ...
                            {/* Sink - Dynamic */}
                            <div className="flex items-center gap-1.5 px-2 py-1 rounded bg-white/5 border border-white/5 group-hover:bg-white/10 transition-colors">
                                {config.sink_type === 'duckdb' ? Icons.DuckDB : config.sink_type === 'minio' ? Icons.MinIO : config.sink_type === 'postgres' ? Icons.Postgres : Icons.Sink}
                                <span className="text-[10px] uppercase font-bold text-gray-400">
                ...
                <div className="flex items-center gap-3 w-full md:w-auto justify-end">
                    ...
                    <div className="relative group/menu">
                        ...
                        <div className="absolute right-0 top-full mt-2 w-48 bg-[#0f111a] border border-white/10 rounded-xl shadow-2xl opacity-0 invisible group-hover/menu:opacity-100 group-hover/menu:visible transition-all z-20 overflow-hidden ring-1 ring-black/50">
                            {/* Run Now Option (Moved here) */}
                            <button
                                onClick={() => onTrigger(config.source_tablename)}
                                disabled={isRunning || triggeringTable === config.source_tablename}
                                className={`w-full text-left px-4 py-2.5 text-xs font-medium flex items-center gap-2 transition-colors ${isRunning ? 'text-gray-500 cursor-not-allowed' : 'text-indigo-400 hover:bg-white/10 hover:text-indigo-300'}`}
                            >
                            ...
                            <button onClick={() => onEdit(config)} className="w-full text-left px-4 py-2.5 text-xs font-medium text-gray-300 hover:bg-white/10 hover:text-white flex items-center gap-2 transition-colors">
            ...
            <div className="bg-black/20 p-4 border-t border-white/5">
                <div className="flex items-center justify-between gap-4 text-[10px] text-gray-500 font-bold uppercase tracking-widest mb-3">
                ...
                <div className="grid grid-cols-3 gap-4">
                    ...
                    <div className="relative">
                        ...
                        <div className="text-xl font-bold text-white font-mono">
                            {selectedTimeRange ? (loadedStats || 0).toLocaleString() : (latestRun?.stages?.find(s => s.pipeline_type === 'loader_source_to_dl')?.rows_processed || 0).toLocaleString()}
                        </div>
                    ...
                    <div className="relative">
                        ...
                        <div className="text-xl font-bold text-white font-mono">
                            {selectedTimeRange ? (loadedStats || 0).toLocaleString() : (latestRun?.stages?.find(s => s.pipeline_type === 'loader_dl_to_sink')?.rows_processed || 0).toLocaleString()}
                        </div>
                        <div className="w-full h-1 bg-gray-800 rounded-full mt-2 overflow-hidden">
